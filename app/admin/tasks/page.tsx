...
function AdminTasksContent() {
  ...
  const departmentF = searchParams.get('department') ?? '전체'
  ...
  const filtered = tasks.filter(t => {
    ...
    if (departmentF !== '전체' && t.department !== departmentF) return false
    const term = search.trim().toLowerCase()
    if (term) {
      const text = [t.title, t.author, t.department].filter(Boolean).join(' ').toLowerCase()
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 space-y-3">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="text-xs text-gray-400 mr-1">부서</span>
          <FilterChip label="전체" active={departmentF === '전체'} onClick={() => updateParam('department', '전체')} />
          {DEPARTMENTS.map(d => (
            <FilterChip key={d} label={d} active={departmentF === d} onClick={() => updateParam('department', d)} />
          ))}
        </div>
      </div>
