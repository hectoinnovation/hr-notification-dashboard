...
export async function POST() {
  ...
  const { error: taskErr } = await supabase.from('ai_tasks').upsert([
    {
      ...
      current_work: '매주 채용공고를 직접 작성하고 있습니다.',
  ...
  if (taskErr) return NextResponse.json({ seeded: false, error: taskErr.message }, { status: 500 })
  ...
  if (commentErr) return NextResponse.json({ seeded: false, error: commentErr.message }, { status: 500 })
