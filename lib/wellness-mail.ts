 ...
 * 감싸 Buffer로 반환한다(생성 로직/옵션 완전히 동일, 결과물만 다운로드 대신 메일 첨부로
 * 전달). /api/debug/txt-attachment-test의 xlsx-small variant로 실제 수신 성공이 확인된
 * 것과도 정확히 동일한 코드다. 컬럼/순서/값은 절대 줄이거나 바꾸지 않는다 — 업체 제출용
 * 파일이라 화면 다운로드 결과와 항상 완전히 같아야 한다.
 */
